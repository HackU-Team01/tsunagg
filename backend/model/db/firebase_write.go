...
func Write_firebase(client *firestore.Client, b []uint8, uuid string, pattern int) {
	// fmt.Println(b)
	...
	if pattern == 1 {
		// データ追加(users)
		_, _, _ = client.Collection("user_sample").Add(context.Background(), map[string]interface{}{
		...
		_, _ = client.Collection("user_info_sample").Doc(uuid).Set(context.Background(), map[string]interface{}{
...
func Make_firebase_Attribute(client *firestore.Client, A_slice []string, uuid string, pattern int) {
	if pattern == 1 {
		// Attributeコレクションにまだ追加されてないAttributeのドキュメントを生成
		for _, v := range A_slice {
			_, _ = client.Collection("Attribute_user_sample").Doc(v).Set(context.Background(), map[string][]string{
				"Applicable_users_id": {uuid},
			}, firestore.MergeAll)
			_, _ = client.Collection("Attribute_user_sample").Doc(v).Set(context.Background(), map[string]string{
				"channel_name": (v + "_channel"),
			}, firestore.MergeAll)
			_, _ = client.Collection("Attribute_user_sample").Doc(v).Set(context.Background(), map[string]int{
...
func Write_firebase_Attribute(client *firestore.Client, user_attribute []string, attribute_doc_name string, uuid string, pattern int) {
	if pattern == 1 {
		//ユーザが持つAttribute
		...
		if interfaceContain(uuid, res) == true {
			...
			_, _ = client.Collection("Attribute_user_sample").Doc(attribute_doc_name).Set(context.Background(), map[string][]interface{}{
				"Applicable_users_id": users_id,
			}, firestore.MergeAll)
			_, _ = client.Collection("Attribute_user_sample").Doc(attribute_doc_name).Set(context.Background(), map[string]int{
...
func Write_firebase_Match(client *firestore.Client, uuid string, match_userid string, match_userid_match_Attribute []string) {
	_, _ = client.Collection("match_user_sample").Doc(uuid).Set(context.Background(), map[string]map[string][]string{
		"match_user_info": {match_userid: match_userid_match_Attribute},
	}, firestore.MergeAll)
	_, _ = client.Collection("match_user_sample").Doc(match_userid).Set(context.Background(), map[string]map[string][]string{
